...
CCamera* CAirplanePlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
		...
		case SPACESHIP_CAMERA:
			...
			SetMaxVelocityY(400.0f);//마찰력을 나타낸다.
	...
	* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, int nMeshes):CPlayer(nMeshes)
{
	...
	//플레이어의 위치를 설정한다.
	SetPosition(XMFLOAT3(0.0f, 1500.0f, -50.0f));
...
CCamera* CTerrainPlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	...
	//3인칭 카메라일 때 플레이어에 y-축 방향으로 중력이 작용한다.
		SetGravity(XMFLOAT3(0.0f, -9050.0f, 0.0f));
		SetMaxVelocityXZ(450.0f);
		SetMaxVelocityY(400.0f);
		m_pCamera = OnChangeCamera(THIRD_PERSON_CAMERA, nCurrentCameraMode);
		m_pCamera->SetTimeLag(0.25f);
		m_pCamera->SetOffset(XMFLOAT3(0.0f, 20.0f, -50.0f));
