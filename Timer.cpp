...
{
	if (::QueryPerformanceFrequency((LARGE_INTEGER*) & m_nPerformaceFrequency))
	{
		m_bHardWareHAsPerformanceCounter = TRUE;
		::QueryPerformanceCounter((LARGE_INTEGER*)&m_nLastTime);
		m_fTimeScale = 1.0f / m_nPerformaceFrequency;//Scale Counter의 양= 1/성능 카운터의 빈도
	}
	else
	{
...
void CGameTimer::Tick(float fLockFPS)
{
	...
	//이 함수의 파라메터(fLockFPS)가 0보다 크면 이 시간만큼 호출한 함수를 기다리게 한다.
		...
		while (fTimeElapsed<(1.0f/fLockFPS))
		{
			...
			{
				m_nCurrentTime = ::timeGetTime();//윈도우 시작후 지금까지 시간
			}
			float fTimeElapsed = (m_nCurrentTime - m_nLastTime) * m_fTimeScale;
		}
	}
...
을 m_fFrameTime[0]에 저장한다. */
	...
	//초당 프레임 수를 1증가시키고 현재 프레임 처리 시간을 누적하여 저장한다.
